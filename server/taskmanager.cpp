...
QString TaskManager::create_task(int taskNumber)
{
    ...
    case 1:
        ...
        taskText += ". Найдите каркас графа. Задайте его рёбра через запятую.";
    ...
    case 3:
        ...
        break;
    case 4:
        taskText = "Дана матрица смежности ориентированного графа из 8 вершин:";
        taskText += create_task4(genKey);
        taskText += "Найти цикл. Задайте ребра цикла через запятую.";
        return QString("get_task") + "$4$" + QString::number(genKey) + "$" + taskText;
        break;
    default:
...
bool TaskManager::check_task(int taskNumber, int taskKey, QString answer)
{
    ...
    case 4:
        return check_task4(taskKey, answer);
    default:
...
QString TaskManager::create_task4(int taskKey) {
    QString result;
    bool table[GRAPH_SIZE * GRAPH_SIZE];

    for (int i = 0; i < GRAPH_SIZE * GRAPH_SIZE; i++)
        table[i] = false;
    for (int i = 0; i < GRAPH_SIZE; i++)
        table[i+((i+1)%GRAPH_SIZE)*GRAPH_SIZE] = true; //деление для зацикливания, что возвратиться в начало. Устанавливает диагональные элементы
    std::srand(taskKey);
    int r = rand() % 8;
    for (int i = 0; i < r; i++) //Запускается цикл для добавления случайных дополнительных рёбер. В каждой итерации выбирается случайная пара вершин
        table[(rand() % GRAPH_SIZE) + (rand() % GRAPH_SIZE) * GRAPH_SIZE] = true; //в матрице смежности, и соответствующий элемент массива table устанавливается в true
    result += "[";
    for (int i = 0; i < GRAPH_SIZE; i++) {
        result += "[";
        for (int j = 0; j < GRAPH_SIZE; j++) {
             result += QString::number(table[j+i*GRAPH_SIZE]) + " ";
        }
        result += "], ";
    }
    result += "]";
...
bool TaskManager::check_task4(int taskKey, QString answer) {
    QStringList edges = answer.split(",");
    QString tableString = create_task4(taskKey);
    bool table[GRAPH_SIZE * GRAPH_SIZE];
    for (int i = 0; i < GRAPH_SIZE * GRAPH_SIZE; i++)
        table[i] = false;
    int row = 0, column = 0;
    for (int i = 0; i < tableString.length(); i++) {
        if (tableString[i].isDigit()) {
            table[column + row * GRAPH_SIZE] = true; //если символ - цифра, устанавливается соответствующий элемент массива table в true
            column += 1;
        } else if (tableString[i] == ',') {
            column = 0;
            row += 1;
        }
    }
    for (int i = 0; i < edges.size(); i++) { //Здесь проверяется, что текущее ребро не нарушает ожидаемую структуру матрицы смежности.
        if (!table[edges[i][0].digitValue()-1 + (edges[i][1].digitValue()-1) * GRAPH_SIZE])
            return false; // сравнивается ребро из графа и введенное ребро
        if (i > 0 and edges[i][0] != edges[i-1][1]) //здесь проверяется, что вершина, начальная для текущего ребра, совпадает с
            //вершиной, завершающей предыдущее ребро. Это делается, чтобы удостовериться, что ребра образуют замкнутый цикл.
            return false;
        if (edges[0][0] != edges[edges.size()-1][1]) //проверяется, что начальная вершина первого ребра совпадает с вершиной,
            //завершающей последнее ребро.
            return false;
    }
    return true;
}
